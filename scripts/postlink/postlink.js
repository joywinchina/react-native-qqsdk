...
function addAppIdToGradle() {
  ...
  var appIDLink = "${QQ_APP_ID}";
    ...
    console.log('App ID 可能已存在，如果要进行修改请在joywinchina-react-native-qqsdk的Gradle文件中手动修改Android App ID');
  }
}
...
function addFrameworkAndSearchPath() {
  ...
  var frameworkPath = path.join(__dirname,'../node_modules/joywinchina-react-native-qqsdk/ios/RCTQQSDK/TencentOpenAPI.framework');
  ...
  var project_relative = path.relative(project_dir, frameworkPath);
    project.parse(function (error) {
      if (error) {
        ...
        addSearchPaths(project,'"$(SRCROOT)/../node_modules/joywinchina-react-native-qqsdk/ios/RCTQQSDK/**"');
